...
OPS = {
    ...
    'STCDC06_3x3x3': lambda C, stride, affine: CDC_Unit(C, C, 3, stride, 1, basic_conv=ST_3DCDC, theta=0.6,
                                                        affine=affine),
    'TCDC06_3x3x3': lambda C, stride, affine: CDC_Unit(C, C, 3, stride, 1, basic_conv=T_3DCDC, theta=0.6,
                                                       affine=affine),
    'TCDC03avg_3x3x3': lambda C, stride, affine: CDC_Unit(C, C, 3, stride, 1, basic_conv=T_3DCDC_Avg, theta=0.3,
                                                          affine=affine),
    'TCDC06_3x1x1': lambda C, stride, affine: CDC_Unit(C, C, 3, stride, padding=1, basic_conv=T_3DCDC, theta=0.6,
                                                       affine=affine),
    'TCDC03avg_3x1x1': lambda C, stride, affine: CDC_Unit(C, C, 3, stride, padding=1, basic_conv=T_3DCDC_Avg, theta=0.3,
                                                          affine=affine),
    'dil_3x3x3': lambda C, stride, affine: DilConv(C, C, 3, stride, padding=2, dilation=2, affine=affine),
    'STCDC06_dil_3x3x3': lambda C, stride, affine: Dil_STCDC(C, C, 3, stride, padding=2, dilation=2, theta=0.6,
                                                             affine=affine),
    'TCDC06_dil_3x3x3': lambda C, stride, affine: Dil_TCDC(C, C, 3, stride, padding=2, dilation=2, theta=0.6,
                                                           affine=affine),
    ...
    # stride = 4
    'TCDC06_5x1x1': lambda C, stride, affine: CDC_Unit(C, C, 5, stride, padding=2, basic_conv=T_3DCDC, theta=0.6,
                                                       affine=affine),
    'TCDC03avg_5x1x1': lambda C, stride, affine: CDC_Unit(C, C, 5, stride, padding=2, basic_conv=T_3DCDC_Avg, theta=0.3,
                                                          affine=affine),
...
OPS_connection = {
    ...
    # stride = 2
    'TCDC06_3x1x1': lambda C_in, C_out, stride, affine: CDC_Unit(C_in, C_out, 3, stride, padding=1, basic_conv=T_3DCDC,
                                                                 theta=0.6, affine=affine),
    'TCDC03avg_3x1x1': lambda C_in, C_out, stride, affine: CDC_Unit(C_in, C_out, 3, stride, padding=1,
                                                                    basic_conv=T_3DCDC_Avg, theta=0.3, affine=affine),
    ...
    'conv_3x1x1': lambda C_in, C_out, stride, affine: VaniConv3d_Temporal_3x1x1(C_in, C_out, 3, stride, 1,
                                                                                affine=affine),
    # stride = 4
    'TCDC06_5x1x1': lambda C_in, C_out, stride, affine: CDC_Unit(C_in, C_out, 5, stride, padding=2, basic_conv=T_3DCDC,
                                                                 theta=0.6, affine=affine),
    'TCDC03avg_5x1x1': lambda C_in, C_out, stride, affine: CDC_Unit(C_in, C_out, 5, stride, padding=2,
                                                                    basic_conv=T_3DCDC_Avg, theta=0.3, affine=affine),
    'conv_5x1x1': lambda C_in, C_out, stride, affine: VaniConv3d_Temporal_5x1x1(C_in, C_out, 5, stride, 2,
                                                                                affine=affine),
        ...
        if math.fabs(self.theta - 0.0) < 1e-8:
            ...
            if self.conv.weight.shape[2] > 1:
                ...
                if self.conv.weight.shape[2] == 5:
                    kernel_diff = self.conv.weight[:, :, 0, :, :].sum(2).sum(2) + self.conv.weight[:, :, 1, :, :].sum(
                        2).sum(2) + self.conv.weight[:, :, 3, :, :].sum(2).sum(2) + self.conv.weight[:, :, 4, :, :].sum(
                        2).sum(2)
                kernel_diff = kernel_diff[:, :, None, None, None]
                out_diff = F.conv3d(input=x, weight=kernel_diff, bias=self.conv.bias, stride=self.conv.stride,
                                    padding=0, groups=self.conv.groups)
            ...
            if self.conv.weight.shape[2] > 1:
                ...
                if self.conv.weight.shape[2] == 5:
                    kernel_diff = self.conv.weight[:, :, 0, :, :].sum(2).sum(2) + self.conv.weight[:, :, 1, :, :].sum(
                        ...
                        2).sum(2)
                kernel_diff = kernel_diff[:, :, None, None, None]
                out_diff = F.conv3d(input=local_avg, weight=kernel_diff, bias=self.conv.bias, stride=1, padding=0,
...
class T_1DCDC(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, kernel_size=3, stride=1,
        ...
        self.conv = nn.Conv3d(in_channels, out_channels, kernel_size=(kernel_size, 1, 1), stride=stride,
                              padding=(padding, 1, 1), dilation=dilation, groups=groups, bias=bias)
        self.theta = theta
...
class Dil_STCDC(nn.Module):
    ...
    def __init__(self, C_in, C_out, kernel_size, stride, padding, dilation, theta, affine=True):
        ...
        self.op = nn.Sequential(
            nn.ReLU(inplace=False),
            ST_3DCDC(C_in, C_out, kernel_size, stride=stride, padding=padding, dilation=dilation, theta=theta,
                     bias=False),
            nn.Conv3d(C_in, C_out, kernel_size=1, padding=0, bias=False),
            nn.BatchNorm3d(C_out, affine=affine),
        )
...
class Dil_TCDC(nn.Module):
    ...
    def __init__(self, C_in, C_out, kernel_size, stride, padding, dilation, theta, affine=True):
        ...
        self.op = nn.Sequential(
            nn.ReLU(inplace=False),
            T_3DCDC(C_in, C_out, kernel_size, stride=stride, padding=padding, dilation=dilation, theta=theta,
                    bias=False),
            nn.Conv3d(C_in, C_out, kernel_size=1, padding=0, bias=False),
            nn.BatchNorm3d(C_out, affine=affine),
        )
